...
(function() {
    ...
    // Function to identify a wider range of clickable elements
    ...
    // Function to walk up the DOM from the click target to find a button candidate
    function findButtonCandidate(startEl){
        let el = startEl, depth = 0;
        while (el && depth < 5){ // Increased depth for flexibility
...
// Add overlay to page
        ...
        selectButtonBtn.addEventListener('click', function() {
            ...
            function handleButtonSelection(e) {
                ...
                const buttonCandidate = findButtonCandidate(e.target);

                if (buttonCandidate) {
                    selectedButton = buttonCandidate; // This is the crucial fix
...
// Create and connect popup to button
    function createPopup(buttonElement, template, options, applyToAll) {
        ...
        if (applyToAll) {
            const significantClass = Array.from(buttonElement.classList).find(c => c.includes('btn') || c.includes('cta'));
            if (significantClass) {
                const allSimilarButtons = iframeDoc.querySelectorAll(`.${significantClass}`);
                // Use a Set to ensure uniqueness and then convert back to an array
                const uniqueButtons = new Set([buttonElement, ...allSimilarButtons]);
                targetButtons.splice(0, targetButtons.length, ...uniqueButtons);
            }
        }
        ...
        targetButtons.forEach(btn => {
            ...
            // If button is an anchor or is wrapped in one, prevent default navigation
            const link = btn.tagName === 'A' ? btn : btn.closest('a');
            if (link) {
                link.setAttribute('href', '#');
                // The main script will handle preventDefault, so no inline handler is needed.
            }
        });
        ...
        if (!iframeDoc.getElementById('modern-popup-script')) {
            ...
            script.textContent = `
                document.addEventListener('click', function(e) {
                    ...
                    if (closeButton) {
                        const popup = closeButton.closest('.modern-popup');
                        if (popup) {
